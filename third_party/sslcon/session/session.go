...
func (cSess *ConnSession) DPDTimer() {
	go func() {
		...
		base.Info("TLSDpdTime:", cSess.TLSDpdTime, "TLSKeepaliveTime:", cSess.TLSKeepaliveTime,
		...
		// Keepalive 间隔：用服务器下发值，兜底 20 秒
		kaTime := cSess.TLSKeepaliveTime
		if kaTime <= 0 {
			kaTime = 20
		}

		dpdTicker := time.NewTicker(time.Duration(dpdTime) * time.Second)
		kaTicker := time.NewTicker(time.Duration(kaTime) * time.Second)

		tlsDpd := proto.Payload{Type: 0x03, Data: make([]byte, 0, 8)}
		dtlsDpd := proto.Payload{Type: 0x03, Data: make([]byte, 0, 1)}
		tlsKA := proto.Payload{Type: 0x07, Data: make([]byte, 0, 8)}

		for {
			select {
			case <-kaTicker.C:
				select {
				case cSess.PayloadOutTLS <- &tlsKA:
				default:
				}
			case <-dpdTicker.C:
				select {
