...
1. 在约定源码提交上运行 `make release`，或由 tag workflow 生成并保留 draft Release。
   使用正式身份、provisioning profile、签名、公证、staple 与完整归档验证门禁。
2. 先完成该版本要求的验收。明确获得发布授权后，在有操作员 `gh` 登录的签名 Mac 上运行
   `make publish RELEASE_TAG=vX.Y.Z`。脚本重新下载并验证真实附件，才将 draft 公开。
3. 同一脚本显式派发更新仓库的 `publish.yml`，等待部署并检查 canonical 清单的版本、
   build、摘要及说明。跨仓库派发使用操作员现有登录，不在仓库或客户端保存 PAT。
4. 更新仓库 workflow 串行运行，从公开 Release 与真实归档生成清单、历史和审计记录，
...
启动或上传。两包签名 Info.plist 的 `XDialUpdateAcceptanceID` 相同，对应固定地址
