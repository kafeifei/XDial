...
for command in tailscale jq; do
  if ! command -v "$command" >/dev/null; then
    print -u2 "$command is unavailable"
    exit 2
  fi
done
...
if [[ -z "$baseline_interface" ]] ||
   [[ "$(jq -r '.BackendState' <<<"$baseline_state")" != "Running" ]] ||
...
dscacheutil -q host -a name "$controlled_host" >/dev/null
curl \
  --silent \
  --show-error \
  --insecure \
...
print -r -- "$probe_logs" | tail -n 60
trial_logs="$(grep -F "flow trial=$trial_id" <<<"$probe_logs" || true)"
if [[ -z "$trial_logs" ]]; then
  print -u2 "no DNS flow from this trial was observed"
  exit 1
fi
if ! grep -F "host=$controlled_host" <<<"$trial_logs" >/dev/null; then
  print -u2 "no domain-attributed flow from this trial was observed"
  exit 1
fi
if ! grep -F "NEAppProxyTCPFlow" <<<"$trial_logs" >/dev/null; then
  print -u2 "no TCP flow from this trial was observed"
  exit 1
fi
