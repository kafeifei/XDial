...
// invariants_test.go —— 把 XDial 约束规范写成 CI 门禁。
//
// 这里的每个测试守护一条约束规范条款，而不是某个函数的实现细节。它们的存在意义是：
	...
	return &Profile{
		...
		RuleSets: []RuleSet{
			...
			{ID: "remote", Name: "REMOTE", Type: RuleSetTypeURL, Enabled: true,
				URL: "https://example.com/remote.srs"},
		},
		Modes: []Mode{{
			ID: "m", Name: "默认模式",
...
// ===========================================================================
//
// 守护：约束规范「内部正交律」+「声明≠生效」。Line/RuleSet/订阅可以存在于 profile 里
...
// ===========================================================================
//
// 守护：约束规范「DNS 原理：名字→线路→地址」+「解析权随线路走」。
...
// ===========================================================================
//
// 守护：约束规范「外部密封律」。桌面配置必须自己劫持并应答 DNS；一旦把原始 DNS 包
...
// ===========================================================================
//
// 守护：约束规范「Mode 是唯一裁决者」+ D28「供给出来的规则必须以一等公民身份进入 Mode
// 裁决，绝不隐式抢注」。
//
...
//   - 任何“默认直连私有网段 / 默认拦截广告”之类的便利规则：约束规范禁止隐式裁决。
...
//
// 守护：约束规范 D28「供给出来的规则必须以一等公民身份进入 Mode 裁决，绝不隐式抢注」。
...
// 实际用本地非权威或被篡改结果去连隧道那头），或者反过来在 dns.rules 里塞了没有路由对应的域名分支。
