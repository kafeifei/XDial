...
func testProfile() *Profile {
	return &Profile{
		...
		Rules: []Rule{
			...
			{ID: "remote", Name: "REMOTE", Type: RuleTypeURL, Enabled: true,
...
func TestProfileFinders(t *testing.T) {
	...
	if r := p.FindRule("remote"); r == nil || r.URL == "" {
		t.Fatal("FindRule(remote) failed")
	}
...
func TestGenerateWithManualRule(t *testing.T) {
	...
	// 海外策略不用 REMOTE rule-set，不应有 rule_set
