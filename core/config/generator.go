...
func generateSingBox(profile *Profile, socksPort int, vpnServerIP string, platform Platform, basePath string, enterpriseDNS []string) ([]byte, error) {
	...
	// Tailscale 标签分两份，对应约束规范的「声明≠生效」（INV1c）：
	//
	...
	// 模式的普通规则排在订阅自带规则之前。
	//
	// 约束规范：Mode 是唯一裁决者，订阅只是"自带规则的供给源"（D28）。订阅那几百条
...
func buildTUNInbound(vpnServerIP string, platform Platform) map[string]interface{} {
	...
	// 路由规则永不求值，DNS 经隧道解出的未非权威或被篡改 AAAA 反而被系统 Happy Eyeballs
...
// bootstrap 解析，加密通道也躲开 UDP 53 明文非权威或被篡改。
...
func buildDNS(
	...
	// 兜底之前：内网名的归属已定，而这里的域名（remotePolicy 等）本就不是 tailnet 名字。
