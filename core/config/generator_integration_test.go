...
func remoteCargo() Cargo {
	return Cargo{
		ID: "remote", Name: "RemotePolicy", Type: CargoTypeURL, Enabled: true,
...
func TestGenerate_ManualOnly(t *testing.T) {
	p := &Profile{
		...
		Cargoes: []Cargo{internalCargo(), remoteCargo(), cnipCargo()},
		Cruises: []Cruise{{
			ID: "c1", Name: "海外",
			Bindings: []Binding{
				{CargoID: "internal", PortID: "vpn"},
				{CargoID: "remote", PortID: "direct"},
...
func TestGenerate_BindingToSubscription(t *testing.T) {
	p := &Profile{
		...
		Cargoes:       []Cargo{internalCargo(), remoteCargo()},
		Subscriptions: []Subscription{testSubscription()},
		Cruises: []Cruise{{
			ID: "c1", Name: "国内",
			Bindings: []Binding{
				{CargoID: "internal", PortID: "vpn"},
				{CargoID: "remote", SubscriptionID: "sub-test"}, // REMOTE → 订阅
			},
			DefaultPortID: "direct",
		}},
		ActiveCruiseID: "c1",
	...
	p := &Profile{
		...
		Cruises: []Cruise{{
			...
			Bindings: []Binding{
				...
				{CargoID: "remote", SubscriptionID: "sub-test"}, // 订阅
...
func TestGenerate_PlainSubscription(t *testing.T) {
	p := &Profile{
		...
		Cargoes:       []Cargo{remoteCargo()},
		Subscriptions: []Subscription{testSubscriptionNoGroups()},
		Cruises: []Cruise{{
			ID: "c1", Name: "Plain",
			Bindings: []Binding{
				{CargoID: "remote", SubscriptionID: "sub-plain"},
			},
			DefaultPortID: "direct",
		}},
		ActiveCruiseID: "c1",
...
func TestGenerate_DisabledSubscription(t *testing.T) {
	...
	p := &Profile{
		...
		Cruises: []Cruise{{
			...
			Bindings: []Binding{
				{CargoID: "internal", PortID: "vpn"},
				{CargoID: "remote", SubscriptionID: "sub-test"}, // 订阅禁用，应跳过
			},
			DefaultPortID: "direct",
		}},
		ActiveCruiseID: "c1",
...
func TestGenerate_EmptySubscription(t *testing.T) {
	...
	p := &Profile{
		...
		Cruises: []Cruise{{
			ID: "c1", Name: "Empty",
			Bindings: []Binding{
				{CargoID: "remote", SubscriptionID: "sub-empty"},
			},
			DefaultPortID: "direct",
		}},
		ActiveCruiseID: "c1",
