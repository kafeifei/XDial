...
- **已知目标归属覆盖全流程**：能按域名或目标 IP 明确匹配 Scenario 绑定时，先确定 Line。
  域名仅使用该绑定授权的解析视角，再经所属 Line 连接；IP 字面量直接按绑定进入所属 Line。
  不得为预解析、目标探测或失败回退先经过无关 Line，也不得混用其他解析视角的缓存答案。
  最终出口正确不能抵消前置解析或验证走错线路；尚需解析才能判定的纯 IP 规则遵守
  `ARCHITECTURE.md` 第 4.1 节的边界，不猜测归属。
...
- **环境兼容性优先设计**：连接准备、DNS 引导、就绪检查和状态展示应尽量避免依赖易受
  路径限制、解析异常或中间设备干扰的方式。逐项审视必要的外部服务及其实际访问路径，
  优先使用符合 Line 职责、经过验证的通信能力，减少建立连接前的外部依赖；不得让某条
  Line 的建立反过来依赖 Direct 已具备该 Line 才能提供的访问能力。固定 IP、公共解析器
  或加密传输本身都不是可达性保证；展示和诊断服务失败不得阻断核心连接流程。
- **探测结论不得越过证据范围**：酒店、企业及其他受限网络中，单个目标的 DNS、TCP 或
  TLS 失败，只能说明该目标在该路径下的访问结果，不能直接判定整条 Line、整个地址族或
  Scenario 不可用。Direct 不得以某个固定外部服务可达作为通用连接前提；固定 IP 或 HTTPS
  不保证可达，替换另一个单点也不能解决判断范围错误。服务检查必须经过实际所属 Line，
  失败影响仅限于依赖该服务的功能；出口 IP 展示探测不得成为连接门禁。兼容环境差异不得
  静默改变 DNS、线路归属或跳过证书认证。
